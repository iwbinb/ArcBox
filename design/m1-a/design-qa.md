...
- **Typography and content:** system Latin/CJK sans, large task-led heading, 16 px body baseline and readable Chinese tool sentences match the selected hierarchy. Funds-related task sentences follow the repository tool rules where the generated mock was imprecise; the prototype omits unsupported live claims and labels every money-related sample as a demo.
...
5. **P1 — misleading money copy and incomplete payment summary:** the generated mock implied Group collects only after reaching the threshold and did not explain that Split distributes eligible revenue. The implementation now says buyers pay for a seat before the deadline and may reclaim principal if the group fails; Split copy describes distributable income. The payment panel visibly reserves positions for tool, order, payer wallet, network, receiving contract, recipient and network fee. The final [payment desktop](screenshots/payment-1440.png) and [payment mobile](screenshots/payment-375.png) captures show these fields without inventing addresses or a successful transaction.
