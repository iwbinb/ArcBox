...
import { CHAIN_ID, EVENT_TOPIC, USDC, ordersConfiguration, type Attempt, type Deployment, type OrderEnv } from './domain';
import { ChainReader, logFingerprint, type ChainTransaction } from './chain-reader';
import { decodeBusiness, incident, ingestVerifiedReceipt, matchesOrder } from './projection';
...
async function saveAttempt(db:D1Database,a:Attempt,status:Attempt['status'],code:string|null,tx:ChainTransaction|null=null):Promise<Attempt>{
  ...
  await db.prepare("UPDATE transaction_attempts SET status=?1,last_error=?2,checks=checks+1,next_check_at=?3,checked_at=?4,sender=COALESCE(?5,sender),tx_nonce=COALESCE(?6,tx_nonce) WHERE id=?7 AND status IN ('QUEUED','PENDING')").bind(status,code,now+delay,now,tx?.from??null,tx?.nonce??null,a.id).run();
  if(tx&&['CONFIRMED','REVERTED'].includes(status)){
...
export async function refreshAttempt(env:OrderEnv,attemptId:string):Promise<Attempt>{
  ...
  try{
    ...
    if(a.purpose==='approval'){
      ...
      // Approval never changes the order projection, even when a token Transfer is present.
    ...
    const quarantined=await env.DB.prepare("SELECT count(*) n FROM order_chain_events e JOIN order_event_outcomes s ON s.event_key=e.event_key WHERE e.deployment_id=?1 AND e.chain_order_id=?2 AND s.state='QUARANTINED'").bind(d.id,o.chain_order_id).first<number>('n');
    return saveAttempt(env.DB,a,'CONFIRMED',quarantined?'PROJECTION_QUARANTINED':null,tx);
  }catch(error){
...
async function collectRange(env:OrderEnv,d:Deployment,reader:ChainReader,from:number,to:number):Promise<{events:number;applied:number}>{
  ...
  for(const l of logs){const key=`${l.transactionHash}:${l.logIndex}`;if(seen.has(key))bad(503,'DUPLICATE_RPC_LOG');seen.add(key);}
  const expected=new Set(logs.map(logFingerprint));let events=0,applied=0;
  for(const hash of hashes){
    const receipt=await reader.receipt(hash);
    if(!receipt||receipt.status!=='success'||receipt.block.number<from||receipt.block.number>to)bad(503,'RPC_LOG_RECEIPT_MISMATCH');
...
export async function scanDeployment(env:OrderEnv,deploymentId:string):Promise<Record<string,unknown>>{
  ...
  try{
    ...
    if(from>latest.number){
      ...
      return {status:'CURRENT',deploymentId,lastCompleteBlock:cursor.last_complete_block};
    }
    let to=Math.min(from+127,latest.number),result:{events:number;applied:number}|null=null;
    for(let attempt=0;attempt<8;attempt++){
      try{result=await collectRange(env,d,reader,from,to);break;}
      catch(error){
        if(error instanceof ApiError&&['RPC_RANGE_LIMIT','RPC_UNAVAILABLE'].includes(error.code)&&to>from){to=from+Math.floor((to-from)/2);continue;}
        throw error;
      }
    }
    if(!result)bad(503,'SCAN_BUDGET_EXHAUSTED');
    ...
    await env.DB.batch([
      ...
      env.DB.prepare('UPDATE order_sync_cursors SET last_complete_block=?1,last_block_hash=?2,last_checked_at=?3,last_error=NULL,lease_token=NULL,lease_until=0,version=version+1 WHERE deployment_id=?4 AND lease_token=?5').bind(to,end.hash,Date.now(),d.id,token),
      releaseAssertion(env.DB,op),
    ]);
    return {status:'SCANNED',deploymentId,fromBlock:from,toBlock:to,...result};
  }catch(error){
...
export async function syncTick(env:OrderEnv):Promise<Record<string,unknown>>{
  ...
  ordersConfiguration(env);
  const due=await env.DB.prepare("SELECT id FROM transaction_attempts WHERE status IN ('QUEUED','PENDING') AND next_check_at<=?1 ORDER BY next_check_at,id LIMIT 2").bind(Date.now()).all<{id:string}>();
  const attempts:unknown[]=[];for(const a of due.results)attempts.push(await refreshAttempt(env,a.id));
  ...
  return {status:'COMPLETE',attempts:attempts.length,scan:next?await scanDeployment(env,next.id):null};
}
